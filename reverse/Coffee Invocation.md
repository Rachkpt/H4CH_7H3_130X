...
## JNI et classes Java embarquées

Le binaire utilise JNI (`JNI_CreateJavaVM`, `libjvm.so`) pour exécuter du code Java embarqué. On trouve deux fichiers `.class` directement dans le binaire, repérables par leur magic number.

```bash
$ xxd coffee_invocation | grep -a "cafebabe"
00005180: cafe babe ...
00005680: cafe babe ...
```
...
Après analyse des tables, inversion du mapping et tri, on obtient la deuxième partie :

```
1v3_h4d_a1l_my_0xCAFEBABE
```
...
- Le flag n'est pas chiffré dans le binaire : c'est l'argument que l'on donne qui est validé puis affichéMarkdown# Coffee Invocation - HTB Reverse Engineering
